...
#np.set_printoptions(suppress=True) #wtf is this?
...
#             print(e)
            ...
            bag_out_rgb.write(depth_data_topic, depth_msg, titimestampme)
            #metadata
        ...
        try:
            ...
            print(f'Failed to get rgb frame {i}:',e)
...
def align_depth_rsrgb_multi(bag_in_path, bag_out_rgb_path, output_folder, aligned_rgb_depth_folder, depth_timestamps, depth_frames, depth_fps, depth_dims, rgb_dims):
    # .bag file for depth info
    try:
        bag_in = rosbag.Bag(bag_in_path)
        bag_out_rgb = rosbag.Bag(bag_out_rgb_path, 'w')
#         print(bag_in_path)
#         print(bag_out_rgb_path)
#         print(depth_fps, depth_dims, rgb_dims)
...
def align_depth_ximea_single(bag_in_path, bag_out_ximea_path, output_folder, aligned_ximea_depth_folder, 
    ...
    for i, (t, frame) in enumerate(zip(depth_timestamps, depth_frames)):
        ...
        try:
            ...
            print(f'Failed to get ximea frame {i}:',e)
    #     finally:
    #         pipe.stop()
...
def align_depth_ximea_mutli(bag_in_path, bag_out_ximea_path, output_folder, aligned_ximea_depth_folder, 
                      depth_timestamps, depth_frames, depth_fps, depth_dims, rgb_dims, ximea_dims,
                      depth_to_ximea_rotation_q, depth_to_ximea_translation,
                      ximea_intrinsics, ximea_distortion, color_metadata_msg, depth_metadata_msg):
    try:
...
# #new function to create aligned depth files one bag file at a time.
# #this is a bit of a hack to solve a temporal alignment issue
# def create_aligned_depth_files_single(recording_folder, output_folder,
#                                ximea_distortion, ximea_intrinsics, 
#                                rgb_distortion, rgb_intrinsics,
#                                rgb_to_ximea_rotation, rgb_to_ximea_translation,bag_in_path='/home/vasha/st-bravo_analysis/bag/sample_final.bag',
#                               skip_rgb=False, skip_ximea=False):
#     '''
#     Run offline alignment of depth stream to both the world camera and the ximea camera coordines.
#     Steps:
#         1. Create a .bag file with depth frames and dummy realsense RGB frames inside. The distortion and intrinsics are those provided by realsense (distortions are zero)
...
#     bag_in_path='/home/vasha/st-bravo_analysis/bag/sample_final.bag',
#     aligned_rgb_depth_folder = os.path.join(output_folder,'rgb_aligned_depth')
#     os.makedirs(aligned_rgb_depth_folder, exist_ok=True)
#     aligned_ximea_depth_folder = os.path.join(output_folder,'ximea_aligned_depth')
#     os.makedirs(aligned_ximea_depth_folder, exist_ok=True)
...
#         bag_out_rgb_path = os.path.join(output_folder,f'depth_rgb_{i}.bag')
#         bag_out_ximea_path = os.path.join(output_folder,f'depth_ximea_{i}.bag')
...
#     print('Finished Creating Depth -> RGB Bag Files')
...
#         pipe = rs2.pipeline()
#         cfg = rs2.config()
#         cfg.enable_device_from_file(os.path.join(output_folder,f'depth_rgb_{i}.bag'))
#         profile = pipe.start(cfg)
...
#         print('Finished Writing RGB Aligned Depth Files - Deleting Bag file!')
#         os.remove(os.path.join(output_folder,f'depth_rgb_{i}.bag'))
...
#                 msg.height = rgb_dims[1]
#                 msg.width = rgb_dims[0]
#                 msg.D = rgb_distortion
#                 msg.K = rgb_intrinsics
#                 bag_out_ximea.write(topic, msg, t)
...
#     #print('Finished Creating Depth -> Ximea Bag Files')

#     ###########
#         #Now use realsense pipeline to read in frame from .bag to run align_to
#         pipe = rs2.pipeline()
#         cfg = rs2.config()
#         cfg.enable_device_from_file(os.path.join(output_folder,f'depth_ximea_{i}.bag'))
#         profile = pipe.start(cfg)
#         playback = profile.get_device().as_playback().set_real_time(False)
...
#         df = aligned_frames.get_depth_frame()
#         aligned_depth_frame = np.array(df.get_data())
#         cv2.imwrite(os.path.join(aligned_ximea_depth_folder,f'depth_frame_{str(framenum).zfill(8)}.png'),
#                             aligned_depth_frame)
#         pipe.stop()
    
#     print('Finished Writing Ximea Aligned Depth Files - Deleting Bag File Now')
#     os.remove(os.path.join(output_folder,f'depth_ximea_{i}.bag'))
