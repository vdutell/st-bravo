...
def createpngfileslist():
    
    #open csv file
    output_csv = './png_file_list.csv'
    with open(output_csv,'w',newline='') as file:
        writer = csv.writer(file)
        writer.writerow(['anadir','subject','task','iter','calfolder','rgb_ximea','nframes'])
        ...
        for triallistlinenumber in range(42):
            ...
            trial_list_path = '~/st-bravo_clean/trial_list.csv'
            ...
            for frames in ['rgb','ximea']:
                frames_folder = os.path.join(anadir,subject_name,task_name,task_iter,f'pngs')
                print(frames_folder)
                
                #get number of frames
                nframes = os.popen(f'ls {frames_folder} | wc -l')
                writer.writerow([anadir,subject_name,task_name, 'task', task_iter, frames, nframes.read()])
            ...
            for task in ['left_eye_cal','right_eye_cal','val_eye_cal']:
                ...
                if calibration_folder not in calibration_folders:
                    ...
                    for frames in ['rgb','ximea']:
                        frames_folder = os.path.join(anadir,subject_name,'calib',calibration_folder,'pngs/')
                        #print(triallistlinenumber, anadir, subject_name, task, frames)
                        #print(frames_folder)

                        #get number of frames
                        nframes = os.popen(f'ls {frames_folder} | wc -l')
                        writer.writerow([anadir,subject_name,task,calibration_folder, 'calib', frames, nframes.read()])
